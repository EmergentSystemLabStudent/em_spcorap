...
basic_stopwords = [
    ...
    "location", "talking", "You're", "We", "refer", "spot", "Welcome", "Certainly!", "called", "Call", "known"
]
...
def ReadWordData(step, trialname, particle):
    ...
    # インクリメントパラメータ追加
    if use_increment_w:
        Otb_BOW = [[count * increment_param for count in row] for row in Otb_BOW]
...
def ParticleSearcher(trialname, data):
    m_count = 0  # m_countの数
    # Count m_count that is number of gmapping output steps
    while (os.path.exists(
            datafolder + trialname + "/particle/{}/".format(data + 1) + str(m_count + 1) + ".csv") == True):
        m_count += 1
...
def SaveMaxLikelihoodParams(filename, max_likelihood_datafile, max_index):
    ...
    shutil.copyfile(filename + "/particle" + str(max_index) + ".csv",
                    max_likelihood_datafile + "/particle" + str(max_index) + ".csv")
...
def callback():
    ...
    for step in tqdm(range(1, N+1)): # 追加学習するときは、この値を追加するデータ分だけ入れる。
        ...
        MAX_weight_particle = np.argmax(
            p_weight)  # p_weight.index(max(p_weight))                     ##最大重みのパーティクルindex
